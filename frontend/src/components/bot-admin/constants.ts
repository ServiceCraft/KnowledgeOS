...
export const SETTING_HINTS = {
  ...
  yclients_booking:
    'Разрешает боту читать данные YClients: услуги, мастеров и свободные слоты — чтобы называть клиенту реальную доступность. Требует заданного секрета YClients (партнёрский токен + company_id).',
  yclients_autobook:
    'Разрешает боту самому создавать запись в YClients. При выключенном тумблере бот показывает слоты и передаёт диалог оператору для ручной записи (режим первой фазы). Работает только вместе с чтением слотов.',
