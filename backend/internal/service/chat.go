...
func buildSystemPrompt(settings *domain.BotSettings, sources []domain.ChatSource, toolDefs []llm.Tool, isFirstReply bool) string {
	...
	if hasYClientsRead {
		...
		b.WriteString("Специалистов бери ТОЛЬКО из yclients_get_staff выбранного филиала — не выдумывай врачей и специальности, которых нет в списке (например, не предлагай «травматолога», если его нет). Если клиент просит «выберите вы» — сам выбери подходящего из списка (для общего осмотра, боли, недомогания — терапевта или врача широкого профиля) и назови конкретного врача.\n")
		b.WriteString("Свободное время бери из yclients_get_times (нужны staff_id и дата). Когда клиент спрашивает «когда можно», «какие даты», «ближайшее», «на этой неделе», «покажите время» — НЕ спрашивай дату в ответ: сам вызови yclients_get_times на нужную/ближайшую дату, а если окон нет — проверь следующие 2–3 дня и покажи конкретные свободные даты и времена. Отказывай или предлагай оператора только если реально ничего нет в ближайшие дни.\n")
	}
	if hasYClientsRead && !hasBooking {
		b.WriteString("Запись (read-only): ты помогаешь подобрать приём и показываешь реальную доступность, а саму запись оформляет оператор. Порядок:\n")
		b.WriteString("1. Определи филиал (сопоставь по названию или уточни, если не сказан).\n")
		b.WriteString("2. Пойми потребность — какое животное и что беспокоит (по одному вопросу за раз, не переспрашивай уже названное).\n")
		b.WriteString("3. Подбери специалиста из yclients_get_staff и назови конкретного врача.\n")
		b.WriteString("4. Покажи свободное время из yclients_get_times по этому врачу на нужную/ближайшую дату (при отсутствии — следующие дни).\n")
		b.WriteString("5. Когда клиент выбрал врача и время (или просит записать) — передай диалог оператору через request_handoff, кратко перечислив: филиал, животное, проблема, врач, желаемые дата и время. Оператор подтвердит и оформит запись.\n")
		b.WriteString("НИКОГДА не предлагай «оставить заявку», «обратный звонок», «администратор/врач перезвонит» — даже если такой ответ есть в базе знаний. Вместо этого показывай доступность через инструменты и передавай оператору.\n")
		b.WriteString("Если ты предложил передать оператору и клиент согласился («да», «давайте») — сразу вызови request_handoff, не переспрашивай. Но не передавай оператору просто потому, что на одну дату нет окон — сначала покажи ближайшие свободные даты.\n")
	}
	if hasBooking {
