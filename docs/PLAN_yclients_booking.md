...
**Статус:** реализовано и смёржено в `B-1` (ветка `feature/yclients-booking`). Осталось боевое подключение: партнёрский токен + `company_id` филиала от заказчика, включение тумблера «Запись в YClients» в настройках бота.
**Ветка-основа:** `B-1`
